...
const registerVoiceMarketplace = require("./src/voice-marketplace").createVoiceMarketplaceService({
  calls: callRepository, marketplace: marketplaceRepository,
  resolveCallerNumber: callSid => exotelCallerLookup.resolveCallerNumber(callSid),
  publish: event => adminEvents.publish(event)
});
...
require("./src/workspace-api").mountWorkspaceApi(app, { db: getDatabase(), marketplaceRepository, extractFarmerData, adminEvents });
...
function findTopMarketplaceMatches(intent, commodity, quantityKg, location, repository = marketplaceRepository, callerNumber = null) {
  const targetSide = intent === "SELL" ? "BUY" : "SELL";
  const candidates = repository.getEligibleListings(targetSide, commodity);
  return rankMatches({ intent, commodity, quantityKg, location, callerNumber }, candidates);
}
...
app.get("/", (_req, res) => res.sendFile(path.join(__dirname, "public", "admin", "index.html")));
...
wss.on("connection", exotelSocket => {
  ...
  async function processSellLocation() {
    ...
    let mandiSpeech = `${session.marketplaceNotice || ""} ${mandiVoiceAnnouncement(discovery, session.commodity, session.location, session.responseLanguage)}`.trim();
    if (rates.length) {
  ...
  async function processBuyLocation() {
    ...
    if (!session.matchedSellers.length) {
      return endConversation(`${session.marketplaceNotice || ""} ${messages.NO_SELLERS_RECORDED} ${messages.THANK_YOU}`.trim());
    }
    ...
    setStage(STAGES.WAITING_FOR_MATCH_ACTION);
    return safeSpeak(
      `${session.marketplaceNotice || ""} ${matchVoiceAnnouncement("BUY", session.matches, session.responseLanguage)} ${messages.BUY_MATCH_SMS_MENU}`.trim()
    );
  }
  ...
  async function finalizeCurrentTurn() {
    ...
    const missing = missingRequiredSlots(session);
    if (!missing.length) {
      const registration = await registerVoiceMarketplace(callSid, { isActive: () => !terminalCallPersisted });
      session.marketplaceNotice = registrationMessage(registration, session.responseLanguage);
      if (registration.callerNumber) session.callerNumber = registration.callerNumber;
      if (terminalCallPersisted) return;
